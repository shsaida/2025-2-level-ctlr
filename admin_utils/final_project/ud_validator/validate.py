...
line_of_first_morpho_feature = (
    None  # features are optional, but if the treebank has features, then some become required
)
...
global_entity_attribute_string = (
    None  # to be able to check that repeated declarations are identical
)
entity_attribute_number = 0  # to be able to check that an entity does not have extra attributes
entity_attribute_index = (
    {}
)  # key: entity attribute name; value: the index of the attribute in the entity attribute list
entity_types = (
    {}
)  # key: entity (cluster) id; value: tuple: (type of the entity, identity (Wikipedia etc.), line of the first mention)
open_entity_mentions = []  # items are dictionaries with entity mention information
open_discontinuous_mentions = (
    {}
)  # key: entity id; describes last part of a discontinuous mention of that entity; item is dict, its keys: last_ipart, npart, line
    ...
    if args.max_err > 0 and error_counter[testclass] > args.max_err:
        ...
        if lineno > 0:
            print(
                ...
                % (fn, lineno, sent, node, testlevel, testclass, testid, msg),
                file=sys.stderr,
            )
        elif lineno < 0:
            print(
                "[%sLine %d%s%s]: [L%d %s %s] %s"
                % (fn, sentence_line, sent, node, testlevel, testclass, testid, msg),
                file=sys.stderr,
            )
        else:
            print(
                "[%sLine %d%s%s]: [L%d %s %s] %s"
                % (fn, curr_line, sent, node, testlevel, testclass, testid, msg),
                file=sys.stderr,
            )
...
def is_multiword_token(cols):
    """
    Checks if token is a multiword.
    """
...
def lspec2ud(deprel):
    """
    Returns left argument of deprel.
    """
...
def trees(inp, tag_sets, args):
    """
    This function does elementary checking of the input and yields one sentence at a time from the input stream.
    ...
    for line_counter, line in enumerate(inp):
        ...
        if is_whitespace(line):
            ...
            testmessage = (
                "Spurious line that appears empty but is not; there are whitespace characters."
            )
            ...
            if lines:  # sentence done
                ...
                testmessage = (
                    "Spurious empty line. Only one empty line is expected after every sentence."
                )
                warn(testmessage, testclass, testlevel, testid)
        elif line[0] == "#":
            ...
            if len(cols) != COLCOUNT:
                ...
                testmessage = (
                    "The line has %d columns but %d are expected. The contents of the columns will not be checked."
                    % (len(cols), COLCOUNT)
                )
                warn(testmessage, testclass, testlevel, testid)
                corrupted = True
            ...
            testid = "invalid-line"
            testmessage = (
                "Spurious line: '%s'. All non-empty lines should start with a digit or the # character."
                % (line)
            )
            warn(testmessage, testclass, testlevel, testid)
    else:  # end of file
...
def validate_unicode_normalization(text):
    ...
    if text != normalized_text:
        ...
        testmessage = "Unicode not normalized: %s.character[%d] is %s, should be %s." % (
            COLNAMES[firsti],
            firstj,
            inpfirst,
            nfcfirst,
...
def validate_cols_level1(cols):
    """
    Tests that can run on a single line and pertain only to the CoNLL-U file format, not to predefined sets of UD tags.
    """
    testlevel = 1
    testclass = "Format"
    ...
    for col_idx in range(MISC + 1):
        ...
        if not cols[col_idx]:
            ...
            if whitespace2_re.match(cols[col_idx]):
                ...
                testmessage = (
                    "Two or more consecutive whitespace characters not allowed in column %s."
                    % (COLNAMES[col_idx])
                )
                warn(testmessage, testclass, testlevel, testid)
    ...
    if is_multiword_token(cols):
        for col_idx in (FORM, LEMMA):
            ...
            if whitespace_re.match(cols[col_idx]):
                ...
                testmessage = (
                    "White space not allowed in multi-word token '%s'. If it contains a space, it is not one surface token."
                    % (cols[col_idx])
                )
                warn(testmessage, testclass, testlevel, testid)
    ...
    for col_idx in (ID, UPOS, XPOS, FEATS, HEAD, DEPREL, DEPS):
        ...
        if whitespace_re.match(cols[col_idx]):
            ...
            testmessage = "White space not allowed in column %s: '%s'." % (
                COLNAMES[col_idx],
                cols[col_idx],
            )
            warn(testmessage, testclass, testlevel, testid)
    ...
    for cols in tree:
        ...
        if is_word(cols):
            ...
            if word_id == current_word_id and tokens and word_id < tokens[-1][0]:
                testid = "misplaced-empty-node"
                testmessage = "Empty node id %s must occur before multiword token %s-%s." % (
                    cols[ID],
                    tokens[-1][0],
                    tokens[-1][1],
                )
...
def validate_sent_id(comments, known_ids, lcode):
    ...
    if not matched:
        ...
        if sid.count("/") > 1 or (sid.count("/") == 1 and lcode != "ud" and lcode != "shopen"):
            ...
            testmessage = (
                "The forward slash is reserved for special use in parallel treebanks: '%s'" % sid
            )
            warn(testmessage, testclass, testlevel, testid)
        known_ids.add(sid)
        ...
        for cols in tree:
            ...
            if (
                len(
                    [
                        x
                        for x in cols[MISC].split("|")
                        if re.match(r"^SpaceAfter=", x) and x != "SpaceAfter=No"
                    ]
                )
                > 0
            ):
            ...
            if not stext.startswith(cols[FORM]):
                ...
                if "SpaceAfter=No" in cols[MISC].split("|"):
                    ...
                    if args.check_space_after and (stext) and not stext[0].isspace():
                        ...
                        warn(
                            testmessage, testclass, testlevel, testid, lineno=sentence_line + iline
                        )
                    stext = stext.lstrip()
        if stext:
...
def validate_cols(cols, tag_sets, args):
    """
    All tests that can run on a single line. Done as soon as the line is read, called from trees() if level>1.
    """
...
def validate_token_empty_vals(cols):
    ...
    for col_idx in range(
        ...
        if col_idx == FEATS and cols[col_idx] == "Typo=Yes":
            ...
            testmessage = "A multi-word token line must have '_' in the column %s. Now: '%s'." % (
                COLNAMES[col_idx],
                cols[col_idx],
            )
            warn(testmessage, testclass, testlevel, testid)
...
def validate_empty_node_empty_vals(cols):
    ...
    This is required by UD guidelines but not necessarily by CoNLL-U, therefore a level 2 test.
    """
...
deprel_re = re.compile(r"^[a-zA-Z]+(:[a-zA-Z]+)?$")
upos_re = re.compile(r"^[A-Z]+$")


def validate_character_constraints(cols):
    """
    Checks general constraints on valid characters, e.g. that UPOS only contains [A-Z].
    """
    testlevel = 2
    if is_multiword_token(cols):
        return
...
def validate_features(cols, tag_sets, args):
    """
    Checks general constraints on feature-value format.

    On level 4 and higher, also checks that a feature-value pair is listed as approved.
    (Every pair must be allowed on level 2 because it could be defined as language-specific.
    ...
    for f in feat_list:
        ...
        if match is None:
            ...
            testmessage = (
                "Spurious morphological feature: '%s'. Should be of the form Feature=Value and must start with [A-Z] and only contain [A-Za-z0-9]."
                % f
            )
            warn(testmessage, testclass, testlevel, testid)
                ...
                if args.level > 3:
                    ...
                    if effective_featset is not None:
                        if attr not in effective_featset:
                            ...
                            testmessage = "Feature %s is not documented for language [%s]." % (
                                attr,
                                effective_lang,
                            )
                            ...
                            if lfrecord["permitted"] == 0:
                                ...
                                testmessage = "Feature %s is not permitted in language [%s]." % (
                                    attr,
                                    effective_lang,
                                )
                                if not altlang and len(warn_on_undoc_feats) > 0:
                                ...
                                if not v in values:
                                    ...
                                    testmessage = (
                                        "Value %s is not documented for feature %s in language [%s]."
                                        % (v, attr, effective_lang)
                                    )
                                    if not altlang and len(warn_on_undoc_feats) > 0:
                                        testmessage += "\n\n" + warn_on_undoc_feats
                                        warn_on_undoc_feats = ""
                                    ...
                                    testmessage = (
                                        "Feature %s is not permitted with UPOS %s in language [%s]."
                                        % (attr, cols[UPOS], effective_lang)
                                    )
                                    if not altlang and len(warn_on_undoc_feats) > 0:
                                        testmessage += "\n\n" + warn_on_undoc_feats
                                        warn_on_undoc_feats = ""
                                    ...
                                    not v in lfrecord["byupos"][cols[UPOS]]
                                    or lfrecord["byupos"][cols[UPOS]][v] == 0
                                ):
                                    testid = "feature-value-upos-not-permitted"
                                    testmessage = (
                                        "Value %s of feature %s is not permitted with UPOS %s in language [%s]."
                                        % (v, attr, cols[UPOS], effective_lang)
                                    )
                                    if not altlang and len(warn_on_undoc_feats) > 0:
                                        testmessage += "\n\n" + warn_on_undoc_feats
                                        warn_on_undoc_feats = ""
...
def features_present():
    """
    Checks first encountered morphological feature.
    ...
    if not line_of_first_morpho_feature:
        ...
        for testid in delayed_feature_errors:
            for occurrence in delayed_feature_errors[testid]["occurrences"]:
                warn(
                    delayed_feature_errors[testid]["message"],
...
def validate_required_feature(feats, fv, testmessage, testlevel, testid, nodeid, lineno):
    """
    Checks required morphological features.
    ...
    if not fv in feats.split("|"):
        if line_of_first_morpho_feature:
            ...
            if not testid in delayed_feature_errors:
                delayed_feature_errors[testid] = {
                    "class": testclass,
                    "level": testlevel,
                    "message": testmessage,
                    "occurrences": [],
                }
            delayed_feature_errors[testid]["occurrences"].append(
                {"nodeid": nodeid, "lineno": lineno}
            )


def validate_upos(cols, tag_sets):
    """
    Checks part of speech tags.
    """
    if UPOS >= len(cols):
        return  # this has been already reported in trees()
...
def validate_deprels(cols, tag_sets, args):
    """
    Checks dependency relation (deprel) tags.
    """
    ...
    if args.level < 4:
        ...
        testlevel = 2
    if deprelset is not None and deprel.lower() not in deprelset:
        testclass = "Syntax"
...
def deps_list(cols):
    """
    Outputs dependency relation as a list.
    """
    if DEPS >= len(cols):
        return  # this has been already reported in trees()
    if cols[DEPS] == "_":
...
def validate_root(tree):
    ...
    for cols in tree:
        if is_word(cols):
            ...
            if cols[HEAD] == "0" and lspec2ud(cols[DEPREL].lower()) != "root":
                testclass = "Syntax"
...
def validate_deps(tree):
    """
    Validates that DEPS is correctly formatted and that there are no self-loops in DEPS.
    """
            ...
            for h, d in deps:
                if h == lasth:
                    if d < lastd:
                        ...
                        testmessage = (
                            "DEPS contain multiple instances of the same relation '%s:%s'" % (h, d)
                        )
                        warn(testmessage, testclass, testlevel, testid, lineno=node_line)
                lasth = h
                lastd = d
    ...
    for cols in tree:
        ...
        for ma in misc:
            ...
            if re.match(r"^(SpaceAfter|Lang|Translit|LTranslit|Gloss|LId|LDeriv)$", ma[0]):
                ...
                r"^\s*(spaceafter|lang|translit|ltranslit|gloss|lid|lderiv)\s*$",
                ma[0],
                re.IGNORECASE,
            ):
                testclass = "Warning"  # warning only
                testid = "misc-attr-typo"
                testmessage = "Possible typo (case or spaces) in MISC attribute '%s=%s'." % (
                    ma[0],
                    ma[1],
                )
                warn(testmessage, testclass, testlevel, testid, lineno=node_line)
...
def build_tree(sentence):
    """
    Takes the list of non-comment lines (line = list of columns) describing a sentence.

    Returns a dictionary with items providing easier access to the
    ...
    if unreachable:
        testid = "non-tree"
        testmessage = "Non-tree structure. Words %s are not reachable from the root 0." % (
...
def build_egraph(sentence):
    ...
    if egraph_exists:
        if not line_of_first_enhanced_graph:
            ...
            if line_of_first_tree_without_enhanced_graph:
                ...
                testmessage = (
                    "Enhanced graph must be empty because we saw empty DEPS on line %s"
        ...
        if not line_of_first_tree_without_enhanced_graph:
            ...
            if line_of_first_enhanced_graph:
                testid = "edeps-only-sometimes"
                testmessage = (
                    "Enhanced graph cannot be empty because we saw non-empty DEPS on line %s"
                    % line_of_first_enhanced_graph
                )
                warn(testmessage, testclass, testlevel, testid, lineno=sentence_line)
        return None
    ...
    if unreachable:
        ...
        testmessage = (
            "Enhanced graph is not connected. Nodes %s are not reachable from any root" % sur
        )
        warn(testmessage, testclass, testlevel, testid, lineno=-1)
        return None
    return egraph
...
def validate_upos_vs_deprel(id, tree):
    """
    For certain relations checks that the dependent word belongs to an expected part-of-speech category.
    ...
    if (
        deprel == "mark"
        and re.match(r"^(NOUN|PROPN|ADJ|PRON|DET|NUM|AUX|INTJ)", cols[UPOS])
        and not "fixed" in childrels
    ):
...
def validate_flat_foreign(id, tree):
    """
    Checks for additional tag flat:foreign.
...
def validate_left_to_right_relations(id, tree):
    """
    Checks orientation of relations.
...
def validate_single_subject(id, tree):
    """
    Checks that no predicate has more than one subject.
    ...
    def is_inner_subject(node):
        """
        Checks dependency relation for inner subject tag.
...
def validate_orphan(id, tree):
    ...
    if deprel == "orphan":
        ...
        if not re.match(r"^(conj|parataxis|root|csubj|ccomp|advcl|acl|reparandum)$", pdeprel):
            ...
            testmessage = "The parent of 'orphan' should normally be 'conj' but it is '%s'." % (
                pdeprel
            )
            warn(testmessage, testclass, testlevel, testid, nodeid=id, lineno=tree["linenos"][id])
    ...
    if re.match(r"^(case|mark|cc|aux|cop|det|fixed|goeswith|punct)$", deprel):
        ...
        for idchild in tree["children"][id]:
            ...
            if re.match(r"^(mark|case)$", pdeprel) and not re.match(
                ...
                testmessage = "'%s' not expected to have children (%s:%s:%s --> %s:%s:%s)" % (
                    pdeprel,
                    idparent,
...
def get_caused_nonprojectivities(id, tree):
    """
    Checks whether a node is in a gap of a nonprojective edge.

    Report true only if the node's parent is not in the same gap. (We use this function to check
...
def get_gap(id, tree):
    """
    Calculates a gap within token IDs sequence.
    """
    iid = int(id)  # just to be sure
    pid = int(tree["nodes"][iid][HEAD])
    if iid < pid:
...
def validate_goeswith_span(id, tree):
    """
    Checks span of goeswith argument.
    ...
    testclass = "Syntax"
    gwchildren = sorted(
        [x for x in tree["children"][id] if lspec2ud(tree["nodes"][x][DEPREL]) == "goeswith"]
    )
    if gwchildren:
        ...
        if gwlist != gwrange:
            testid = "goeswith-gap"
            testmessage = "Violation of guidelines: gaps in goeswith group %s != %s." % (
                str(gwlist),
                str(gwrange),
            )
            warn(testmessage, testclass, testlevel, testid, nodeid=id, lineno=tree["linenos"][id])
        ...
        nospaceafter = [
            x for x in gwlist[:-1] if "SpaceAfter=No" in tree["nodes"][x][MISC].split("|")
        ]
        if nospaceafter:
...
def validate_goeswith_morphology_and_edeps(id, tree):
    """
    If a node has the 'goeswith' incoming relation, it is a non-first part of a mistakenly interrupted word.

    The lemma, upos tag and morphological features of the word should be annotated at the first part, not here.
    """
    testlevel = 3
    if lspec2ud(tree["nodes"][id][DEPREL]) == "goeswith":
        ...
        if tree["nodes"][id][LEMMA] != "_":
            ...
            testmessage = (
                "The lemma of a 'goeswith'-connected word must be annotated only at the first part."
            )
            warn(testmessage, testclass, testlevel, testid, nodeid=id, lineno=tree["linenos"][id])
        ...
        testclass = "Enhanced"
        if (
            tree["nodes"][id][DEPS] != "_"
            and tree["nodes"][id][DEPS] != tree["nodes"][id][HEAD] + ":" + tree["nodes"][id][DEPREL]
        ):
...
def validate_fixed_span(id, tree):
    ...
    """
    fxchildren = sorted(
        [i for i in tree["children"][id] if lspec2ud(tree["nodes"][i][DEPREL]) == "fixed"]
    )
    if fxchildren:
...
def validate_projective_punctuation(id, tree):
    ...
    if deprel == "punct":
        ...
        if gap:
            ...
            testmessage = (
                "Punctuation must not be attached non-projectively over nodes %s" % sorted(gap)
            )
            warn(testmessage, testclass, testlevel, testid, nodeid=id, lineno=tree["linenos"][id])
...
def validate_enhanced_annotation(graph):
    """
    Checks universally valid consequences of the annotation guidelines in the enhanced representation.
    ...
    for id in graph.keys():
        ...
        if "orphan" in udeprels:
            ...
            if line_of_first_empty_node:
                ...
                warn(
                    testmessage, testclass, testlevel, testid, nodeid=id, lineno=graph[id]["lineno"]
                )
...
def validate_auxiliary_verbs(cols, children, nodes, line, lang, auxlist):
    """
    Verifies that the UPOS tag AUX is used only with lemmas that are known to act as auxiliary verbs or particles in the given language.
    ...
    Args:
      ...
      lang: language
      auxlist: list of auxiliaries in given language
    """
    if cols[UPOS] == "AUX" and cols[LEMMA] != "_":
        ...
        if not lspecauxs:
            ...
            testid = "aux-lemma"
            testmessage = (
                "'%s' is not an auxiliary in language [%s] (there are no known approved auxiliaries in this language)"
                % (cols[LEMMA], lang)
            )
...
def validate_copula_lemmas(cols, children, nodes, line, lang, coplist):
    """
    Verifies that the relation cop is used only with lemmas that are known to act as copulas in the given language.
    ...
    Args:
      ...
      line: line number of the node within the file
      lang: language
      coplist: list of copulas in given language
    """
...
def validate_misc_entity(comments, sentence):
    """
    Optionally checks the well-formedness of the MISC attributes that pertain to coreference and named entities.
    """
        ...
        if global_entity_match:
            ...
            if line_of_global_entity:
                if global_entity_match.group(1) != global_entity_attribute_string:
                    ...
                    testmessage = (
                        "New declaration of global.Entity '%s' does not match the first declaration '%s' on line %d."
                        % (
                            global_entity_match.group(1),
                            global_entity_attribute_string,
                            line_of_global_entity,
                        )
                    )
                    warn(
                        testmessage, testclass, testlevel, testid, lineno=comment_start_line + iline
                    )
            else:
                ...
                if not re.match(r"^[a-z]+(-[a-z]+)*$", global_entity_attribute_string):
                    ...
                    testmessage = "Cannot parse global.Entity attribute declaration '%s'." % (
                        global_entity_attribute_string
                    )
                    warn(
                        testmessage, testclass, testlevel, testid, lineno=comment_start_line + iline
                    ...
                    if not "eid" in global_entity_attributes:
                        testid = "spurious-global-entity"
                        testmessage = (
                            "Global.Entity attribute declaration '%s' does not include 'eid'."
                            % (global_entity_attribute_string)
                        )
                        warn(
                            testmessage,
                            ...
                            testlevel,
                            testid,
                            lineno=comment_start_line + iline,
                        )
                    elif global_entity_attributes[0] != "eid":
                        testid = "spurious-global-entity"
                        testmessage = (
                            "Attribute 'eid' must come first in global.Entity attribute declaration '%s'."
                            % (global_entity_attribute_string)
                        )
                        warn(
                            testmessage,
                    ...
                    if not "etype" in global_entity_attributes:
                        testid = "spurious-global-entity"
                        testmessage = (
                            "Global.Entity attribute declaration '%s' does not include 'etype'."
                            % (global_entity_attribute_string)
                        )
                        warn(
                            testmessage,
                        ...
                        testid = "spurious-global-entity"
                        testmessage = (
                            "Attribute 'etype' must come second in global.Entity attribute declaration '%s'."
                            % (global_entity_attribute_string)
                        )
                        warn(
                            testmessage,
                    ...
                    if not "head" in global_entity_attributes:
                        testid = "spurious-global-entity"
                        testmessage = (
                            "Global.Entity attribute declaration '%s' does not include 'head'."
                            % (global_entity_attribute_string)
                        )
                        warn(
                            testmessage,
                        ...
                        testid = "spurious-global-entity"
                        testmessage = (
                            "Attribute 'head' must come third in global.Entity attribute declaration '%s'."
                            % (global_entity_attribute_string)
                        )
                        warn(
                            testmessage,
                    ...
                    if (
                        "other" in global_entity_attributes
                        and global_entity_attributes[3] != "other"
                    ):
                        testid = "spurious-global-entity"
                        testmessage = (
                            "Attribute 'other', if present, must come fourth in global.Entity attribute declaration '%s'."
                            % (global_entity_attribute_string)
                        )
                        warn(
                            testmessage,
                    ...
                    for a in global_entity_attributes:
                        if a in entity_attribute_index:
                            testid = "spurious-global-entity"
                            testmessage = (
                                "Attribute '%s' occurs more than once in global.Entity attribute declaration '%s'."
                                % (a, global_entity_attribute_string)
                            )
                            warn(
                                testmessage,
                                testclass,
    ...
    for cols in sentence:
        ...
        if "-" in cols[ID] and (len(entity) > 0 or len(bridge) > 0 or len(splitante) > 0):
            testid = "entity-mwt"
            testmessage = (
                "Entity or coreference annotation must not occur at a multiword-token line."
            )
            warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
            continue
        if len(entity) > 1:
            testid = "multiple-entity-statements"
            testmessage = "There can be at most one 'Entity=' statement in MISC but we have %s." % (
                str(misc)
            )
            warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
            continue
        if len(bridge) > 1:
            ...
            testmessage = "There can be at most one 'Bridge=' statement in MISC but we have %s." % (
                str(misc)
            )
            warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
            continue
        if len(splitante) > 1:
            ...
            testmessage = (
                "There can be at most one 'SplitAnte=' statement in MISC but we have %s."
                % (str(misc))
            )
            warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
            continue
        if len(bridge) > 0 and len(entity) == 0:
            ...
            testmessage = (
                "The 'Bridge=' statement can only occur together with 'Entity=' in MISC but we have %s."
                % (str(misc))
            )
            warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
            continue
        if len(splitante) > 0 and len(entity) == 0:
            ...
            testmessage = (
                "The 'SplitAnte=' statement can only occur together with 'Entity=' in MISC but we have %s."
                % (str(misc))
            )
            warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
            continue
        ...
        if len(entity) > 0:
            if not line_of_global_entity:
                ...
                testmessage = (
                    "No global.Entity comment was found before the first 'Entity' in MISC."
                )
                warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
                continue
                ...
                for b, e in entities:
                    ...
                    if b == 0 or b == 2:
                        ...
                        if len(attributes) > entity_attribute_number:
                            ...
                            testmessage = (
                                "Entity '%s' has %d attributes while only %d attributes are globally declared."
                                % (e, len(attributes), entity_attribute_number)
                            )
                            warn(
                                testmessage,
                                testclass,
                        ...
                        if len(attributes) > 1:
                            testid = "too-many-entity-attributes"
                            testmessage = (
                                "Entity '%s' has %d attributes while only eid is expected at the closing bracket."
                                % (e, len(attributes))
                            )
                            warn(
                                testmessage,
                    ...
                    if match:
                        ...
                        if ipart == 1 and npart == 1:
                            ...
                            testmessage = (
                                "Discontinuous mention must have at least two parts but it has one in '%s'."
                                % (beid)
                            )
                            warn(
                                testmessage,
                        ...
                        if ipart > npart:
                            testid = "spurious-entity-id"
                            testmessage = (
                                "Entity id '%s' of discontinuous mention says the current part is higher than total number of parts."
                                % (beid)
                            )
                            warn(
                                testmessage,
                        ...
                        if re.match(r"[\[\]]", beid):
                            testid = "spurious-entity-id"
                            testmessage = (
                                "Entity id '%s' contains square brackets but does not have the form used in discontinuous mentions."
                                % (beid)
                            )
                            warn(
                                testmessage,
                    ...
                    def opening_bracket():
                        ...
                        if npart > 1:
                            ...
                            attributes_without_eid = [
                                attributes[i]
                                for i in range(len(attributes))
                                if i != entity_attribute_index["eid"]
                            ]
                            ...
                            if ipart == 1:
                                ...
                                if eidnpart in open_discontinuous_mentions:
                                    open_discontinuous_mentions[eidnpart].append(
                                        discontinuous_mention
                                    )
                                else:
                                ...
                                if eidnpart in open_discontinuous_mentions:
                                    discontinuous_mention = open_discontinuous_mentions[eidnpart][
                                        -1
                                    ]
                                    if ipart != discontinuous_mention["last_ipart"] + 1:
                                        testid = "misplaced-mention-part"
                                        testmessage = (
                                            "Unexpected part of discontinuous mention '%s': last part was '%d/%d' on line %d."
                                            % (
                                                beid,
                                                discontinuous_mention["last_ipart"],
                                                discontinuous_mention["npart"],
                                                discontinuous_mention["last_part_line"],
                                            )
                                        )
                                        warn(
                                            testmessage,
                                        ...
                                        testmessage = (
                                            "Attribute mismatch of discontinuous mention: current part has '%s', first part '%s' was at line %d."
                                            % (
                                                attrstring_to_match,
                                                discontinuous_mention["attributes"],
                                                discontinuous_mention["first_part_line"],
                                            )
                                        )
                                        warn(
                                            testmessage,
                                    ...
                                    testid = "misplaced-mention-part"
                                    testmessage = (
                                        "Unexpected part of discontinuous mention '%s': this is part %d but we do not have information about the previous parts."
                                        % (beid, ipart)
                                    )
                                    warn(
                                        testmessage,
                        ...
                        if eid in entity_ids_other_documents:
                            ...
                            testmessage = (
                                "Same entity id should not occur in multiple documents; '%s' first seen on line %d, before the last newdoc."
                                % (eid, entity_ids_other_documents[eid])
                            )
                            warn(
                                testmessage,
                                testclass,
                        ...
                        identity = ""
                        if (
                            "etype" in entity_attribute_index
                            and len(attributes) >= entity_attribute_index["etype"] + 1
                        ):
                            ...
                            if not re.match(
                                r"^(person|place|organization|animal|plant|object|substance|time|number|abstract|event|other)?$",
                                etype,
                            ):
                        ...
                        if (
                            "identity" in entity_attribute_index
                            and len(attributes) >= entity_attribute_index["identity"] + 1
                        ):
                        ...
                        head = 0
                        if (
                            "head" in entity_attribute_index
                            and len(attributes) >= entity_attribute_index["head"] + 1
                        ):
                            if not re.match(
                                r"^[1-9][0-9]*$", attributes[entity_attribute_index["head"]]
                            ):
                                testid = "spurious-mention-head"
                                testmessage = (
                                    "Entity head index '%s' must be a non-zero-starting integer."
                                    % (attributes[entity_attribute_index["head"]])
                                )
                                warn(
                                    testmessage,
                                    testclass,
                        ...
                        if not eid in entity_types:
                            ...
                            if etype != entity_types[eid][0]:
                                ...
                                testmessage = (
                                    "Entity '%s' cannot have type '%s' that does not match '%s' from the first mention on line %d."
                                    % (eid, etype, entity_types[eid][0], entity_types[eid][2])
                                )
                                warn(
                                    testmessage,
                                    testclass,
                            ...
                            if identity != entity_types[eid][1]:
                                ...
                                testmessage = (
                                    "Entity '%s' cannot have identity '%s' that does not match '%s' from the first mention on line %d."
                                    % (eid, identity, entity_types[eid][1], entity_types[eid][2])
                                )
                                warn(
                                    testmessage,
                                    testclass,
                        ...
                        mention = {
                            "beid": beid,
                            "line": sentence_line + iline,
                            "span": [cols[ID]],
                            "text": cols[FORM],
                            "length": 1,
                            "head": head,
                            "attrstring": attrstring_to_match,
                        }
                    ...
                    def closing_bracket():
                        ...
                        if len(open_entity_mentions) == 0:
                            ...
                            testmessage = (
                                "Cannot close entity '%s' because there are no open entities."
                                % (beid)
                            )
                            warn(
                                testmessage,
                            ...
                            if beid != open_entity_mentions[-1]["beid"]:
                                ...
                                testmessage = (
                                    "Entity mentions are not well nested: closing '%s' while the innermost open entity is '%s' from line %d: %s."
                                    % (
                                        beid,
                                        open_entity_mentions[-1]["beid"],
                                        open_entity_mentions[-1]["line"],
                                        str(open_entity_mentions),
                                    )
                                )
                                warn(
                                    testmessage,
                            ...
                            for i in reversed(range(len(open_entity_mentions))):
                                ...
                                testid = "ill-nested-entities"
                                testmessage = (
                                    "Cannot close entity '%s' because it was not found among open entities: %s"
                                    % (beid, str(open_entity_mentions))
                                )
                                warn(
                                    testmessage,
                                    testclass,
                        ...
                        if npart > 1:
                            ...
                            if eidnpart in open_discontinuous_mentions:
                                ...
                                warn(
                                    testmessage, "Internal", 0, testid, lineno=sentence_line + iline
                                )
                                discontinuous_mention = {
                                    "last_ipart": ipart,
                                    "npart": npart,
                        ...
                        if ipart == npart:
                            if mention_length < head:
                                ...
                                testmessage = (
                                    "Entity mention head was specified as %d on line %d but the mention has only %d nodes."
                                    % (head, opening_line, mention_length)
                                )
                                warn(
                                    testmessage,
                                    testclass,
                            ...
                            if ending_mention_key in ending_mentions:
                                ...
                                testmessage = (
                                    "Entity mentions '%s' and '%s' from line %d have the same span %s."
                                    % (
                                        ending_mentions[ending_mention_key],
                                        beid,
                                        opening_line,
                                        str(mention_span),
                                    )
                            ...
                            if eid in entity_mention_spans:
                                if sentid in entity_mention_spans[eid]:
                                    for m in entity_mention_spans[eid][sentid]:
                                        ...
                                        if (
                                            ms.intersection(myset)
                                            and not ms.issubset(myset)
                                            and not myset.issubset(ms)
                                        ):
                                            testid = "crossing-mentions-same-entity"
                                            testmessage = (
                                                "Mentions of entity '%s' have crossing spans: %s vs. %s."
                                                % (eid, m, str(mention_span))
                                            )
                                            warn(
                                                testmessage,
                                                testclass,
                    ...
                    if b == 0:
                        if seen2 and not seen1:
                            testid = "spurious-entity-statement"
                            testmessage = (
                                "If there are no closing entity brackets, single-node entity must follow all opening entity brackets in '%s'."
                                % (entity[0])
                            )
                            warn(
                                testmessage,
                        ...
                        if seen0 and seen2:
                            testid = "spurious-entity-statement"
                            testmessage = (
                                "Single-node entity must either precede all closing entity brackets or follow all opening entity brackets in '%s'."
                                % (entity[0])
                            )
                            warn(
                                testmessage,
                        ...
                        if seen1 and not seen0:
                            testid = "spurious-entity-statement"
                            testmessage = (
                                "If there are no opening entity brackets, single-node entity must precede all closing entity brackets in '%s'."
                                % (entity[0])
                            )
                            warn(
                                testmessage,
                        ...
                        if seen0:
                            testid = "spurious-entity-statement"
                            testmessage = (
                                "All closing entity brackets must precede all opening entity brackets in '%s'."
                                % (entity[0])
                            )
                            warn(
                                testmessage,
                    ...
                    for b in bridges:
                        ...
                        if match:
                            ...
                            if srceid == tgteid:
                                ...
                                testmessage = (
                                    "Bridge must not point from an entity to itself: '%s'." % (b)
                                )
                                warn(
                                    testmessage,
                                    testclass,
                            ...
                            if not tgteid in starting_mentions:
                                ...
                                testmessage = (
                                    "Bridge relation '%s' must be annotated at the beginning of a mention of entity '%s'."
                                    % (b, tgteid)
                                )
                                warn(
                                    testmessage,
                            ...
                            if bridgekey in srctgt:
                                ...
                                testmessage = (
                                    "Bridge relation '%s' must not be repeated in '%s'."
                                    % (bridgekey, b)
                                )
                                warn(
                                    testmessage,
                            ...
                            if bridgekey in entity_bridge_relations:
                                if relation != entity_bridge_relations[bridgekey]["relation"]:
                                    ...
                                    testmessage = (
                                        "Bridge relation '%s' type does not match '%s' specified earlier on line %d."
                                        % (
                                            b,
                                            entity_bridge_relations[bridgekey]["relation"],
                                            entity_bridge_relations[bridgekey]["line"],
                                        )
                                    )
                                    warn(
                                        testmessage,
                                ...
                                entity_bridge_relations[bridgekey] = {
                                    "relation": relation,
                                    "line": sentence_line + iline,
                                }
            if len(splitante) > 0:
                match = re.match(
                    r"^SplitAnte=([^(< :>)]+<[^(< :>)]+(,[^(< :>)]+<[^(< :>)]+)*)$", splitante[0]
                )
                if not match:
                    ...
                    for a in antecedents:
                        ...
                        if match:
                            ...
                            if srceid == tgteid:
                                ...
                                testmessage = (
                                    "SplitAnte must not point from an entity to itself: '%s'."
                                    % (srceid + "<" + tgteid)
                                )
                                warn(
                                    testmessage,
                                ...
                                testmessage = (
                                    "SplitAnte relation '%s' must be annotated at the beginning of a mention of entity '%s'."
                                    % (a, tgteid)
                                )
                                warn(
                                    testmessage,
                            ...
                            if srceid + "<" + tgteid in srctgt:
                                ...
                                testmessage = (
                                    "SplitAnte relation '%s' must not be repeated in '%s'."
                                    % (srceid + "<" + tgteid, ",".join(antecedents))
                                )
                                warn(
                                    testmessage,
                                    testclass,
                    ...
                    for tgteid in tgtante:
                        if len(tgtante[tgteid]) == 1:
                            ...
                            testmessage = (
                                "SplitAnte statement '%s' must specify at least two antecedents for entity '%s'."
                                % (",".join(antecedents), tgteid)
                            )
                            warn(
                                testmessage,
                                testclass,
                        ...
                        if tgteid in entity_split_antecedents:
                            if tgtante[tgteid] != entity_split_antecedents[tgteid]["antecedents"]:
                                ...
                                testmessage = (
                                    "Split antecedent of entity '%s' does not match '%s' specified earlier on line %d."
                                    % (
                                        tgteid,
                                        entity_split_antecedents[tgteid]["antecedents"],
                                        entity_split_antecedents[tgteid]["line"],
                                    )
                                )
                                warn(
                                    testmessage,
                            ...
                            entity_split_antecedents[tgteid] = {
                                "antecedents": str(tgtante[tgteid]),
                                "line": sentence_line + iline,
                            }
        iline += 1
    if len(open_entity_mentions) > 0:
        ...
        testmessage = (
            "Entity mentions must not cross sentence boundaries; still open at sentence end: %s."
            % (str(open_entity_mentions))
        )
        warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
    ...
    if len(open_discontinuous_mentions) > 0:
        ...
        testmessage = (
            "Entity mentions must not cross sentence boundaries; still open at sentence end: %s."
            % (str(open_discontinuous_mentions))
        )
        warn(testmessage, testclass, testlevel, testid, lineno=sentence_line + iline)
...
def validate(inp, out, args, tag_sets, known_sent_ids):
    """
    Does 5 stages of data validation.
    """
...
def load_feat_set(filename_langspec, lcode):
    ...
    if not lcode in featdata:
        ...
        msg += "The following %d feature values are currently permitted in language [%s]:\n" % (
            len(sorted_documented_features),
            lcode,
        )
...
def load_deprel_set(filename_langspec, lcode):
    ...
    if len(deprelset) == 0:
        ...
        msg += "The following %d relations are currently permitted in language [%s]:\n" % (
            len(sorted_documented_relations),
            lcode,
        )
...
def load_edeprel_set(filename_langspec, lcode, basic_deprels):
    ...
    if len(edeprelset) == 0:
        msg += (
            "No enhanced dependency relation types (case markers) have been permitted for language [%s].\n"
            % (lcode)
        )
        msg += "They can be permitted at the address below (if the language has an ISO code and is registered with UD):\n"
        msg += (
...
def load_set(f_name_ud, f_name_langspec, validate_langspec=False, validate_enhanced=False):
    """
    Loads a list of values from the two files, and returns their set.
    ...
    if lcode == "shopen":
        for lcode1 in auxdata.keys():
            ...
            auxlist = auxlist + [
                x
                for x in lemmalist
                if len([y for y in auxdata[lcode1][x]["functions"] if y["function"] != "cop.PRON"])
                > 0
            ]
            coplist = coplist + [
                x
                ...
                if len(
                    [
                        y
                        for y in auxdata[lcode1][x]["functions"]
                        if re.match(r"^cop\.", y["function"])
                    ]
                )
                > 0
            ]
        ...
        auxlist = [
            x
            for x in lemmalist
            if len([y for y in auxdata[lcode][x]["functions"] if y["function"] != "cop.PRON"]) > 0
        ]
        coplist = [
            x
            for x in lemmalist
            if len(
                [y for y in auxdata[lcode][x]["functions"] if re.match(r"^cop\.", y["function"])]
            )
            > 0
        ]
...
def get_alt_language(misc):
    """
    Checks additional language tag for token.
...
if __name__ == "__main__":
    opt_parser = argparse.ArgumentParser(
        description="CoNLL-U validation script. Python 3 is needed to run it!"
    )
    ...
    io_group.add_argument(
        "input", nargs="*", help='Input file name(s), or "-" or nothing for standard input.'
    )
    ...
    list_group.add_argument(
        ...
        required=True,
        default=None,
        help="Which langauge are we checking? If you specify this (as a two-letter code), the tags will be checked using the language-specific files in the data/ directory of the validator.",
    )
    list_group.add_argument(
        "--level",
        action="store",
    ...
    coref_group = opt_parser.add_argument_group(
    ...
    if args.level < 1:
        print(
            "Option --level must not be less than 1; changing from %d to 1" % args.level,
            file=sys.stderr,
        )
        args.level = 1
    ...
    if args.lang:
        ...
        tagsets[TOKENSWSPACE] = [
            re.compile(regex) for regex in tagsets[TOKENSWSPACE]
